...
.DESCRIPTION
    ...
    outputs/screening/SCREENING_RESULTS_<timestamp>.md.

    The script continues past per-run failures so a partial sweep still
    produces a timestamped report.
...
# ----------------------------------------------------------------------------
# Step 9: Aggregate everything into a timestamped SCREENING_RESULTS file.
...
Invoke-Step "Step 11/11: Aggregate diagnostics into timestamped SCREENING_RESULTS.md" {
...
Write-Host "  outputs/screening/SCREENING_RESULTS_<timestamp>.md"
Write-Host "  SCREENING_RESULTS_<timestamp>.md (repo-root copy)"
Write-Host ""
