# -*- coding: utf-8 -*-
"""Build a PLAIN Word version of the 3-state supervisor report (.docx).

Same content and figures as build_report_docx.py, but stripped of the decorative
"formatting visualizations": no shaded callout boxes (green/orange/blue), no
metric-card boxes, no shaded table headers. The only styling is colour on the
titles and sub-titles; body text is plain. Full 32-participant cohort numbers.
"""
...
FIGS = DIR / "figures"
OUT = DIR / "EEG_State_Report_plain.docx"
...
def subhead(doc, text, color=ACCENT, size=12):
    """A coloured, bold sub-title (run-in) - replaces the old shaded callout label."""
    p = doc.add_paragraph(); p.paragraph_format.space_before = Pt(8); p.paragraph_format.space_after = Pt(2)
    r = p.add_run(text); r.bold = True; r.font.size = Pt(size); r.font.color.rgb = RGBColor.from_string(color)
    return p
...
def note(doc, label, lines, color=ACCENT):
    """Plain replacement for the boxed callout: a coloured sub-title + plain paragraphs."""
    subhead(doc, label, color)
    for ln in lines:
        para(doc, ln)

def metrics_plain(doc, cards):
    """Plain replacement for the metric-card boxes: a bulleted line per headline number."""
    for big, lab in cards:
        p = doc.add_paragraph(style='List Bullet'); p.paragraph_format.space_after = Pt(3)
        r = p.add_run(big + "  "); r.bold = True; r.font.size = Pt(14); r.font.color.rgb = RGBColor.from_string(BLUE)
        rr = p.add_run("- " + lab); rr.font.size = Pt(11)

def plain_table(doc, headers, rows, widths):
    """Bordered table with no shading - a coloured, bold header row on white."""
    tbl = doc.add_table(rows=1, cols=len(headers)); tbl.alignment = WD_TABLE_ALIGNMENT.CENTER
    set_table_width(tbl, widths)
    for i, htext in enumerate(headers):
        c = tbl.cell(0, i); set_cell_margins(c)
        cell_borders(c, {e: ('CCD6E0', 6) for e in ('top', 'left', 'bottom', 'right')})
...
def build():
    ...
    metrics_plain(doc, [("0.88", "average score (macro-AUC), where 0.50 = chance"),
    ...
    note(doc, "A note on referencing - CSD (surface Laplacian)",
    ...
    plain_table(doc, ["Feature set", "What's included", "Macro-AUC", "3-way accuracy"], [
    ...
    note(doc, "Two clean answers",
         ["**Does the new foot-SEP feature add anything?** No. Combined and Window score identically "
          ...
          "across the full cohort the brain-source contribution is small but consistent."], color=GREEN)
    ...
    note(doc, "Read this before over-interpreting standing",
         ["Near-perfect detection of standing should not be read as near-perfect decoding of a *cortical* "
          ...
          "artifact-matched controls) is the natural next check."], color=WARN)
