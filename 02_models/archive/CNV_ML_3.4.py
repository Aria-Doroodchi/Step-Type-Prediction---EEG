...
P01_One_epochs = mne.read_epochs('P01_CNV_One-epo.fif')

ba_names = [label.name for label in labels]

epoch_nums = P01_One_epochs.selection.tolist()

LORETA_epoch_rows = []

for idx in range(len(epoch_nums)):
    num = epoch_nums[idx]
    sub_epochs = P01_One_epochs[[idx]]
    ...
    for col in ba_names:
        ...
        bm_temp_transposed.columns = [f'{col}_bin_{b}' for b in bm_temp_transposed.columns]
