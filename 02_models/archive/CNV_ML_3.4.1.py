...
reg_alpha_list = [0, 0.1, 0.5, 1.0, 5.0]
gamma_list = [0, 0.1, 0.3, 0.5]
colsample_bytree_list = [0.6, 0.7, 0.8]
colsample_bylevel_list = [0.6, 0.8, 1.0]
...
for id in participant_ids:
    ...
    for cond in conditions: 
        ...
        for idx, num in enumerate(epoch_nums):
            ...
            bm_activity_df = bm_activity_df[bm_activity_df['time'] >= 0]
    ...
    for max_depth_n in max_depth_list:
        for min_child_weight_n in min_child_weight_list:
            for reg_alpha_n in reg_alpha_list:
                for gamma_n in gamma_list:
                    for colsample_bytree_n in colsample_bytree_list:
                        for colsample_bylevel_n in colsample_bylevel_list:
                            for learning_rate_n in learning_rate_list:
                                    ...
                                    learning_rate=learning_rate_n,        # small LR pairs well with early stopping
                                    max_depth=max_depth_n,               # you can later tune 4–6
                                    min_child_weight=min_child_weight_n,      # a touch more conservative after selection
                                    subsample=0.8,
                                    colsample_bytree=colsample_bytree_n,
                                    reg_lambda=3.0,            # slightly stronger L2
                                    reg_alpha=0.5,             # L1 for sparsity in splits
                                    gamma=gamma_n,                   # regularization on splits
                                    scale_pos_weight=scale_pos_weight,
                                    objective='binary:logistic',
                                    eval_metric='auc',
                                    ...
                                    print(f'Participant {id} classification report for parameter set:')

                                    print(f'learning_rate: {learning_rate_n}, max_depth: {max_depth_n}')
                                    print(f'min_child_weight: {min_child_weight_n}, gamma: {gamma_n}') 
                                    print(f'colsample_bytree: {colsample_bytree_n}')
...
report_df.to_csv('CNV_ML_3.4_report.csv', index=False)
