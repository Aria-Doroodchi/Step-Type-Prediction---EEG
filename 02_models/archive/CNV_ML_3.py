...
for id in participant_ids:
   ...
   # 1) Features/target
    ...
    xgb = XGBClassifier(
        n_estimators=1000,           # large cap; early stopping will cut it
        learning_rate=0.05,          # smaller LR pairs well with early stopping
        ...
        reg_lambda=1.0,              # L2
        reg_alpha= 1,               # L1
        gamma=0.5,                   # split gain threshold
        scale_pos_weight=scale_pos_weight,
        objective='binary:logistic',
        eval_metric='auc',
        early_stopping_rounds=100,           # monitor AUC for early stopping
    ...
    print(f'\nParticipant {id} classification report:')
