...
degree_list = [2, 3, 4]  # only for poly kernel
...
for id in participant_ids:
    ...
    # 3) Class imbalance handling (class_weight for SVM)
    pos = (y_train == 1).sum()
    neg = (y_train == 0).sum()
    
    # Calculate class weights
    class_weights = {0: pos / neg, 1: 1.0} if neg > 0 else {0: 1.0, 1: 1.0}

    # 4) RFECV for feature selection with SVM
    # Use a simpler SVM for feature selection to speed up the process
    rfecv_base = SVC(
        kernel='linear',
        C=1.0,
    ...
    # splits the dataset into 2 folds for cross-validation
    ...
    # Scale features before RFECV (required for SVM)
            ...
            for gamma in gamma_list:
                ...
                if kernel == 'poly':
                    for degree in degree_list:
                        ...
                        proba_test = svm_model.predict_proba(X_test_sel_scaled)[:, 1]
                        pred_test = svm_model.predict(X_test_sel_scaled)
                        ...
                        print(f'Accuracy One: {acc_one:.3f}, Accuracy Two: {acc_two:.3f}, Overall Accuracy: {overall_accuracy:.3f}, AUC: {auc:.3f}')
                        
                else:  # rbf or linear kernel
                    svm_model = SVC(
                        kernel=kernel,
                        C=C,
...
report_df.to_csv('CNV_SVM_report.csv', index=False)
...
print(f'\nMean AUC: {report_df["auc"].mean():.3f}')
print(f'\nBest performing kernel distribution:')
print(report_df['kernel'].value_counts())