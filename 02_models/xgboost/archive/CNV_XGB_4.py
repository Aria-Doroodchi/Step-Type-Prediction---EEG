...
min_time_var = 1 #onset time for predictions. must be betweeen 0 and 2

bin_n = 1/24 # over a 2 second window 
...
for id in participant_ids:
    ...
    n_estimators=2000,          # large cap, early stopping will trim
    subsample=0.8,
    reg_lambda=3.0,
    scale_pos_weight=scale_pos_weight,
    objective="binary:logistic",
    eval_metric="auc",
    tree_method="hist",         # 'gpu_hist' if available
    random_state=42,
    n_jobs= 16
    )
...
report_df.to_csv('CNV_ML_3.6_report.csv', index=False)
