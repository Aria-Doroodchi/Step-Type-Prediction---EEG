...
# importing df
df <- read_csv('../Participants/CNV_ML_3.7_report.csv')
...
accuracy_viollin <- accuracy_df |> 
  ...
  ggplot(aes(x = Condition, y = Accuracy, fill = Condition)) +
    ...
    geom_line(aes(group = participant_id, color = participant_id),
            color = "gray40", alpha = 0.6, linewidth = 0.5) +
...
accuracy_viollin <- accuracy_df |> 
  ...
  geom_line(aes(group = participant_id, color = factor(participant_id)),
            alpha = 0.6, linewidth = 0.5) +
  
  geom_jitter(aes(color = factor(participant_id)),
...
# feature importance 
feature_df <- read.csv('../Participants/top_50_features_per_participant.csv')

feature_df_top <- feature_df |> 
  group_by(feature) |>
...
feature_list <- feature_df_top$feature
sum(str_detect(feature_list, "Theta"))
