...
# rule-dependent (transductive) variant, reported separately
ALIGN_RD=${ALIGN_RD:-online-64:riemann}
P="$HOME/codabench/analysis/sealed_personal.py"
echo "| $(date +%T) | config ALIGN_R=$ALIGN_R ALIGN_E=$ALIGN_E ALIGN_RD=$ALIGN_RD |" >> "$STATUS"
...
laneA() {
  ...
  step t_rx_rd 8 45m python "$P" --tag $TAG --study tangermann2012 --family riemann:xd=1,fb=1 --align $ALIGN_RD
  step z_rx_rd 3 30m python "$P" --tag $TAG --study zhou2016 --family riemann:xd=1,fb=1 --align $ALIGN_RD
...
laneB() {
  ...
  step s_rx_rd 12 60m python "$P" --tag $TAG --study scherer2015 --family riemann:xd=1,fb=1 --align $ALIGN_RD
...
[[ $LANES == *B* ]] && { laneB & }
wait
for s in t_rx t_rn z_rx z_rn t_rx_rd z_rx_rd z_e t_e s_rx s_rn s_rx_rd s_e; do
