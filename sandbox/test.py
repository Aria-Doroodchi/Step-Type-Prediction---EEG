...
final_df = pd.read_csv('test_df.csv')
...
shap_model.fit(X_trained_shap, y_train)
...
proba_test_pruned = pruned_model.predict_proba(X_test_pruned)[:, 1]
...
temp_df_pruned = pd.DataFrame({
    ...
    "best_max_depth": [pruned_model.get_params()["max_depth"]],
    "best_min_child_weight": [pruned_model.get_params()["min_child_weight"]],
    "best_reg_alpha": [pruned_model.get_params()["reg_alpha"]],
    "best_gamma": [pruned_model.get_params()["gamma"]],
    "best_colsample_bytree": [pruned_model.get_params()["colsample_bytree"]],
    "best_colsample_bylevel": [pruned_model.get_params()["colsample_bylevel"]],
    "best_learning_rate": [pruned_model.get_params()["learning_rate"]],
})
...
print(grid.best_params_)  # or pruned_model.get_params() if you prefer
print(
...
tn_p, fp_p, fn_p, tp_p = cm_shap.ravel()
...
acc_one_p = tn_p / total_One_p if total_One_p > 0 else 0.0
acc_two_p = tp_p / total_Two_p if total_Two_p > 0 else 0.0
auc_p = roc_auc_score(y_test, proba_test_shap)
overall_accuracy_p = (tn_p + tp_p) / (total_One_p + total_Two_p) if (total_One_p + total_Two_p) > 0 else 0.0
